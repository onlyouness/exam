...
<body>
    ...
    <div class="relative mx-auto w-4/5 my-4 overflow-x-auto mt-20"> 
        <h1 class="text-gray-800 font-bold text-2xl underline mb-5">{{$voitures->count()}} Voitures:</h1>
        ...
        <table class="w-full text-sm text-left rtl:text-right mt-5 text-gray-500 ">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b  ">
                    ...
                    <td class="px-6 py-4">
                        {{$voiture->num_assurance}}
                    </td>
                    <td class="px-6 py-4 flex justify-center items-center gap-3">
                        <a href="/editVoiture/{{$voiture->id}}"><i class="fa-solid fa-pen-to-square text-blue-600"></i></a>
                        <form method="POST" class="m-0" action="/deleteVoiture/{{$voiture->id}}">
                            @csrf
                            {{method_field("DELETE")}}
